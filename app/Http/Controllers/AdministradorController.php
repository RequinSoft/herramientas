...
class AdministradorController extends Controller
{
    ...
    public function index_admin(){
        ...
        // Valor Inventario
        $valorInventario = Article::query()->whereIn('status',['Disponible', 'Asignado', 'En Reparacion', 'Recibido'])->sum('precio_actual');
        // Artículos Robados
        ...
        //$cat0 = Category::where('status', 'activo')->whereNot('category', 'Default')->get();

        $inventory = Inventory::with('category')->get();
        $categorias = $inventory->pluck('category.category')->toArray();
        $totalArticulos = $inventory->pluck('quantity')->toArray();
        $totalArticulosDinero = $inventory->pluck('investment')->toArray();
        $articulosDisponiblexCategoria = $inventory->pluck('available')->toArray();
        //return $available;

        //Artículos por Status por mes en el año
        $bajas = EquipDown::all();
        
        $articulosRobadosxMes = $bajas->groupBy('month')->map->sum('robados')->toArray();
        $articulosRobadosxMes = array_values($articulosRobadosxMes);
        $articulosExtraviadosxMes = $bajas->groupBy('month')->map->sum('extraviados')->toArray();
        $articulosExtraviadosxMes = array_values($articulosExtraviadosxMes);
        $articulosBajasxMes = $bajas->groupBy('month')->map->sum('bajas')->toArray();
        $articulosBajasxMes = array_values($articulosBajasxMes);

        $articulosBajasxMes_Moneda = $bajas->groupBy('month')->map->sum('bajas_moneda')->toArray();
        $articulosBajasxMes_Moneda = array_values($articulosBajasxMes_Moneda);
        $articulosRobadosxMes_Moneda = $bajas->groupBy('month')->map->sum('robados_moneda')->toArray();
        $articulosRobadosxMes_Moneda = array_values($articulosRobadosxMes_Moneda);
        $articulosExtraviadosxMes_Moneda = $bajas->groupBy('month')->map->sum('extraviados_moneda')->toArray();
        $articulosExtraviadosxMes_Moneda = array_values($articulosExtraviadosxMes_Moneda);
                
        return view('admin.index', compact('ruta', 'totalArticulos', 'totalArticulosDinero', 'valorInventario', 'articuloRobado', 'esteMes', 'categorias', 'articulosDisponiblexCategoria', 'articulosRobadosxMes', 'articulosExtraviadosxMes', 'articulosBajasxMes', 'articulosAsignadosxMes', 'articulosRobadosxMes_Moneda', 'articulosExtraviadosxMes_Moneda', 'articulosBajasxMes_Moneda'));
    }
    ...
    public function personal_crear(Request $request){
        ...
        $nombre_personal = strtolower($request->nombre);
        $nombre_personal = ucwords($nombre_personal);
        //return $request;
        ...
        $ruta = '../storage/app/avatars/';
        //$update_id = Personal::query()->where(['id' => 1])->update(request(['id'])); 
        ...
        if(request()->hasFile('image')){
            ...
            $update = Personal::query()->where(['id' => $request->id])->update(['ext'=>$imagen->guessExtension()]);
        }        

        return  redirect()->to('/admin_personal')->with('user_add', $nombre_personal);
    }
    ...
    public function personal_actualizar(PersonalEditRequest $request){
        ...
        if($existe > 0 && $id_old != $existe1[0]->id){
            ...
            $nombre_personal = strtolower(request('nombre'));
            $nombre_personal = ucwords($nombre_personal);
            $updateuser = Personal::query()->where(['id' => $id_viejo])->update(['id'=>request('id'), 'nombre'=>$nombre_personal, 'puesto'=>request('puesto'), 'group_id'=> request('group_id'), 'status'=>request('status')]);
    ...
    public function articulo_index(){
        ...
        $datos = Article::with('category', 'lineas')->whereNotIn('status', ['Danado', 'Baja'])->get();
    ...
    public function crear_resguardopdf_linea($id, $person_id){
        ...
        $linea = Line::find($id);
        $articulos_grupo = Article::find($linea->article_id);

        $person = Personal::find($person_id);

        $articulos = Line::with('articulos', 'usuario')->where('id', $id)->get();
