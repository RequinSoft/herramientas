...
class CoadminController extends Controller
{
    ...
    public function index_admin(){
        ...
        $inventory = Inventory::with('category')->where('group_id', $grupo[0]->id)->get();
        ...
        $articulosAsignadoxCategoria = $inventory->pluck('assigned')->toArray();
        $articulosDisponiblexCategoria = $inventory->pluck('available')->toArray();
        //return $articulosAsignadoxCategoria;

        $bajas = EquipDown::query()
            ->where('group_id', $id_group)
            ->where('year', $esteAno)
            ->get();

        //$articulosRobadosxMes = $bajas->pluck('robados')->toArray();
    ...
    public function crear_resguardopdf_linea($id, $person_id){
        ...
        $pdf = PDF::loadView('layouts.pdf_coadmin', compact('logged_user', 'group_user', 'articulos', 'ruta', 'person', 'suma', 'empresa', 'hoy', 'depto', 'departamento'));
        return $pdf->stream($person->nombre.'.pdf');
